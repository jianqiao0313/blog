...
function updateScrollProgress() {
  ...
  if (myBar)
    myBar.style.width = `${height > 0 ? (winScroll / height) * 100 : 0}%`;
}
...
function initLightbox(article) {
  ...
  function close(immediate = false) {
    ...
    const remove = () => {
      el.remove();
      if (closingOverlay === el) closingOverlay = null;
    };
...
function initArticle() {
  ...
  cleanupLightbox = initLightbox(article);
}
